...
const geminiApiKey = defineSecret('AQ.Ab8RN6JnYqyov_YTq0LiCSHDPLgBH6Dqv1ggBdvAjDcdq7YUsw');
...
app.post('/chat', async (req, res) => {
  ...
  try {
    const firstResponse = await fetch(`${GEMINI_URL}?key=${geminiApiKey.value()}`, {
      method: 'POST',
    ...
    if (functionCallPart) {
      ...
      const secondResponse = await fetch(`${GEMINI_URL}?key=${geminiApiKey.value()}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          contents: [
...
exports.chatbot = onRequest({ secrets: [AQ.Ab8RN6JnYqyov_YTq0LiCSHDPLgBH6Dqv1ggBdvAjDcdq7YUsw] }, app);